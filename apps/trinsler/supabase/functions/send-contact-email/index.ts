...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Send email using Resend API (direct fetch like in coin app)
    const emailContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #243463; border-bottom: 2px solid #243463; padding-bottom: 10px;">
          Neue Kontaktanfrage
        </h2>
        ...
        <div style="background-color: #f5f5f5; padding: 20px; border-radius: 8px; margin: 20px 0;">
          ...
          ${company ? `<p><strong>Organisation:</strong> ${company.trim()}</p>` : ''}
        </div>

        <div style="background-color: #fff; padding: 20px; border: 1px solid #ddd; border-radius: 8px;">
          <h3 style="margin-top: 0; color: #243463;">Projekt-Idee:</h3>
          <p style="white-space: pre-wrap; line-height: 1.6;">${message.trim()}</p>
        </div>

        <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #ddd; font-size: 12px; color: #666;">
          <p>Diese Nachricht wurde über das Kontaktformular auf trinsler.com gesendet.</p>
          <p>Zeitstempel: ${new Date().toLocaleString('de-DE')}</p>
          ${contactSubmission ? `<p>Eintrags-ID: ${contactSubmission.id}</p>` : ''}
          <p>Sie können direkt auf ${email.trim()} antworten.</p>
        </div>
      </div>
    `
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'noreply@coin.trinsler.com',
        to: 'info.trinsler@gmail.com',
        subject: `Kontaktanfrage: ${name.trim()}`,
        html: emailContent,
        replyTo: email.trim()
      }),
    })
